...
作者：李施施 2014-07-22
**************************************************************************************************/
CTicketActionListMgt::CTicketActionListMgt()
{
	m_count = 0;
	m_sumCount = MAX_TICKETACTIONLIST_COUNT;
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::retrieve(  )
{
    ...
    const char *sql = "SELECT * FROM t_ticketactionlist ORDER BY `index`";
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::retrieve( const DWORD32 ID )
{
	if(!m_isInit)		return -2;
	...
	int loadCount = 0;
	m_count = 0;
    const char *sql = "SELECT * FROM t_ticketactionlist where ticketID=:f<unsigned int>  ORDER BY `index`";
...
作者：刘远安 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::retrieveByID(const DWORD32 ID)
{
    if(!m_isInit)		return -2;
    ...
    const char *sql = "SELECT * FROM t_ticketactionlist where ID=:f<unsigned int> ";
...
作者：刘高舜 2014-08-29
**************************************************************************************************/
int CTicketActionListMgt::retrieveByTicketID( const DWORD32 ID )
{
	if(!m_isInit)		return -2;
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::add( TTicketActionList *pInputParam )
{
	if(!m_isInit)	return -2;
    ...
    sprintf(sql,"insert into t_ticketactionlist values(NULL, \
            :f1_ticketID<unsigned int>, \
            :f2_index<unsigned int>, \
            :f3_devID<unsigned int>, \
            :f4_iscomp<unsigned int>, \
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::del( const DWORD32 ID )
{
	if(!m_isInit)	return -2;
	...
	const char *sql = "DELETE FROM t_ticketactionlist WHERE ID = :f1_ID<unsigned int>";
	try
	{
 ...
 * 作者:  佘佳明 2015-1-12
 ***********************************************************/
int CTicketActionListMgt::delByTicketID(const DWORD32 ticketID)
{
    if(!m_isInit)	return -2;

    const char *sql = "DELETE FROM t_ticketactionlist WHERE ticketID = :f1_ticketID<unsigned int>";
    try
    {
        otl_stream delStream(1, sql, m_db);
        delStream << ticketID;
 ...
 * 作者:  佘佳明 2015-3-11
 ***********************************************************/
int CTicketActionListMgt::delRDataAction(DWORD32 ticketID)
{
    if(!m_isInit)	return -2;

    const char *sql = "DELETE FROM t_ticketactionlist WHERE ticketID=:f1_ticketID<unsigned int> AND isTicketItem='0' AND isKeyItem>'0'";
    try
    {
        otl_stream delStream(1, sql, m_db);
        delStream << ticketID;
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::update( const DWORD32 ID, TTicketActionList *pInputParam )
{
	...
	char *sql = (char *)malloc(2000);
    sprintf(sql,"UPDATE t_ticketactionlist set ticketID=:f1_ticketID<unsigned int>, \
            ...
            operateTime='%s', \
            isMonitor=:f9_ismoni<unsigned int>,  \
            isTicketItem=:f10_isticketitem<unsigned int>,  \
            isKeyItem=:f11_iskeyitem<unsigned int>,  \
            keyItemIndex=:f12_keyitemindex<unsigned int>,  \
            isLinkItem=:f13_islinkitem<unsigned int>,  \
            linkItemIndex=:f14_linkitemindex<unsigned int>,  \
            actionType=:f15_actionType<unsigned int>, \
            WBID=:f16_WBID<unsigned int>,   \
            lockCode=:f17_lockCode<unsigned int> where ID=:f18_id<unsigned int>",
            operateTime);
	try
	{
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::add2List( TTicketActionList *pInputParam )
{
	if(NULL == pInputParam)		return -1;
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::delFromList( const DWORD32 ID )
{
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::isIDExist( const DWORD32 ID )
{
	int index = -1;
...
作者：李施施 2014-07-21
**************************************************************************************************/
int CTicketActionListMgt::getMoreMemory()
{
	int sumCount = m_sumCount + MAX_TICKETACTIONLIST_COUNT;
...
作者：李施施 2014-07-29
**************************************************************************************************/
int CTicketActionListMgt::getListCount()
{
	return m_count;
}
...
作者：李施施 2014-07-29
**************************************************************************************************/
TTicketActionList * CTicketActionListMgt::getListByIndex( DWORD32 index )
{
...
作者：佘佳明 2014-10-24
**************************************************************************************************/
const TTicketActionList *CTicketActionListMgt::getListByID(const DWORD32 ID)
{
...
作者：李施施 2014-07-30
**************************************************************************************************/
int CTicketActionListMgt::updateList( const DWORD32 ID, const TTicketActionList *pInputParam )
{
	if(NULL == pInputParam)		return -1;
