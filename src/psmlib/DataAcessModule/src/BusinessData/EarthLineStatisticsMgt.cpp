...
作者：佘佳明 2015-2-4
**************************************************************************************************/
int CEarthLineStatisticsMgt::retrieveByPileID(const DWORD32 pileID)
{
    if(!m_isInit)		return -2;
    ...
    const char *sql = "SELECT * FROM t_earthlinestatistics where pileID=:f_ID<unsigned int> ";
...
int CEarthLineStatisticsMgt::retrieve(DWORD32 stationID, DWORD32 earthLineID, DWORD32 pileID)
{
	...
	const char *sql = "SELECT * FROM t_earthlinestatistics where stationID=:f1<unsigned int> and earthLineID=:f2<unsigned int> and pileID=:f3<unsigned int>";
...
作者：佘佳明 2015-3-2
**************************************************************************************************/
int CEarthLineStatisticsMgt::delByELAndPile(DWORD32 stationID, DWORD32 GID, DWORD32 ELID, DWORD32 pileID)
{
    if(!m_isInit)	return -2;

    if( checkDBConnect() < 0 )      return -4;

    const char *sql = "DELETE FROM t_earthlinestatistics WHERE \
            stationID=:f1_SID<unsigned int> AND                 \
            groupID=:f2_GID<unsigned int> AND                   \
            earthLineID=:f3_ELID<unsigned int> AND              \
            pileID=:f4_PID<unsigned int>";
    try
    {
        otl_stream delStream(1, sql, m_db);
        delStream << stationID << GID << ELID << pileID;
